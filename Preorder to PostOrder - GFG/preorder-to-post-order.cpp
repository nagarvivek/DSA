...
Node* create(int pre[] , int min , int max ,int &i , int size)
{
    if(i==size || pre[i] > max)
    return NULL;
    ...
    root->left = create(pre , min , root->data , i , size);
    
    root->right = create(pre , root->data , max , i , size);
...
public:
    ...
    {
        int i = 0;
        return create(pre , INT_MIN , INT_MAX , i , size);
